...
public sealed class AsyncSingleton<T> where T : class, new()
{
    ...
    /// <summary>
    /// Provides a thread-safe, asynchronous, and lazy-loaded singleton instance for a specified type <typeparamref name="T"/>.
    /// </summary>
    /// <typeparam name="T">
    ...
    /// </param>
    /// <returns>
    /// A task representing the asynchronous operation, which on completion contains the singleton instance of type <typeparamref name="T"/>.
    /// </returns>
    ...
    public static async Task<T> GetInstanceAsync(Func<Task<T>>? instanceFactory = null)
    {
        ...
        try
        {
            ...
            var task = instanceFactory is not null ? instanceFactory() : DefaultFactory!();
